...
- ✅ **슬라이스 10**: 충돌(맵 경계·건물 통과 불가) + Y-정렬(건물 뒤로 가림)
- ✅ **슬라이스 11**: 목장(양·돼지·오리·토끼) + 가공품(마요네즈·천·오리마요·깃털장식·꿀) + 깊은 광물(마·오팔·시리우스·이리듐) + 시리우스 주괴·하이퍼 보석
- ✅ **슬라이스 12**: 속성의 돌 7종 + 곡괭이 강화(대장장이) + 상점 구매 탭 + 낚시(강) + NPC(백수·이웃) + 토캣몬 인트로
- 🎉 기획서의 주요 시스템 대부분 구현 완료
- ✅ **슬라이스 13(플레이테스트 개선)**: 세이브/로드(진행 유지), HUD·상점 스크롤(오버플로 수정), 광산 바닥 재채굴, 난이도 상향(보석 공물·곡괭이·연구비 ↑, 채집 쿨다운 ↑), 숨겨진 초기화 버튼(우상단 흐릿한 R)
